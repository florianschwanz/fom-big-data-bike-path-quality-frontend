    ...
    if (this.map != null) {
      ...
      results.forEach(name => {
        ...
        this.map.addSource(name,
          {
            type: 'geojson',
            data: baseUrl  + 'data/measurements/geojson/' + name + '.geojson'
          }
        );

        // Download styling for result
        this.http.get(baseUrl + 'data/measurements/styles/style.json', {responseType: 'text' as 'json'}).subscribe((data: any) => {
